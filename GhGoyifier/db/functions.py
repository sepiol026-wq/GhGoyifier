...
from GhGoyifier.db import models
from GhGoyifier.secret_store import decrypt, digest, encrypt, is_encrypted


class User(models.User):
    def __getattribute__(self, name):
        value = super().__getattribute__(name)
        return decrypt(value) if name == "token" else value

    def __setattr__(self, name, value):
        if name == "token" and value is not None:
            value = encrypt(value)
    ...
    async def register(cls, telegram_id):
        await cls.get_or_create(telegram_id=telegram_id)
...
class Chat(models.Chat):
    ...
    async def ensure_registered(cls, chat_id: int) -> "Chat":
        chat, _ = await cls.get_or_create(chat_id=chat_id)
    ...
    @classmethod
    async def get_topic(cls, chat_id: int) -> int | None:
        chat = await cls.get_or_none(chat_id=chat_id)
...
class Integration(models.Integration):
    def __getattribute__(self, name):
        value = super().__getattribute__(name)
        return decrypt(value) if name == "integration_token" else value

    def __setattr__(self, name, value):
        if name == "integration_token" and value is not None:
            value = encrypt(value)
        super().__setattr__(name, value)
    ...
    async def get_by_token(cls, token: str) -> list:
        rows = await cls.filter(integration_token_hash=digest(token)).prefetch_related(
            "chat", "user"
        )
        if rows:
            return rows
        legacy = await cls.filter(integration_token_hash=None).prefetch_related(
            "chat", "user"
        )
        return [row for row in legacy if row.integration_token == token]
