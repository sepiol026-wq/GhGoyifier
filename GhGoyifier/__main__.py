...
from GhGoyifier.proxy import ProxyManager
...
async def main():
    ...
    proxy_manager = ProxyManager(app, arguments.config)
    try:
        await proxy_manager.refresh()
    except Exception:
        logging.getLogger("goyifi.proxy").exception("Initial proxy setup failed; continuing with fallback retry")
    proxy_task = asyncio.create_task(proxy_manager.run())
    bot = GoyBot(app, buttons_mode=config.settings.buttons)
    bot.proxy_manager = proxy_manager
...
if __name__ == "__main__":
    try:
        if not sys.argv[1:] or sys.argv[1] in {"config", "gateway", "logs", "doctor", "proxy", "status", "update", "uninstall", "--help", "-h", "-v", "--version"}:
